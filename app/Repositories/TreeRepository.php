...
final class TreeRepository
{
    ...
    public function findPaginatedTreeInstances(
        float $lat,
        float $lng,
        float $radius,
        int $perPage
        ...
        $query = TreeInstance::query()
            ->where('status', \App\Enums\TreeStatusEnum::ADOPTABLE->value)
            ->with(['tree.media', 'tree.planPrices.plan', 'location'])
            ->whereHas('location', function ($loc) use ($haversine, $radius): void {
                $loc->selectRaw(sprintf('locations.*, %s as distance', $haversine))
                    ->having('distance', '<=', $radius);
            });
    ...
    public function findInstanceById(int $id): ?TreeInstance
    {
        return TreeInstance::query()
            ->where('id', $id)
            ->with(['tree.media', 'tree.planPrices.plan', 'location'])
            ->first();
    }
