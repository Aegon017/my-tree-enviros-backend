...
final class TreeController extends Controller
{
    ...
    public function sponsorship(Request $request): JsonResponse
    {
        $query = Tree::query()
            ->with(['planPrices.plan'])
            ->where('is_active', true)
            ->whereHas('planPrices.plan', function ($query): void {
        ...
        if ($request->filled('location_id')) {
            ...
            if ($location) {
                ...
                $query->whereHas('instances', function ($q) use ($locationIds): void {
    ...
    public function plans(string $treeId, Request $request): JsonResponse
    {
        ...
        if ($request->has('type')) {
            $query->whereHas('plan', function ($q) use ($request): void {
